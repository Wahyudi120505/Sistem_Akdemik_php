...
// Ambil data mata kuliah yang sudah diambil mahasiswa
$query_krs = "SELECT id_mata_kuliah FROM krs WHERE id_mahasiswa = ?";
$stmt_krs = $conn->prepare($query_krs);
$stmt_krs->bind_param("i", $mahasiswa['id']);
$stmt_krs->execute();
$result_krs = $stmt_krs->get_result();

$mata_kuliah_terambil = [];
while ($row_krs = $result_krs->fetch_assoc()) {
    $mata_kuliah_terambil[] = $row_krs['id_mata_kuliah'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($mata_kuliah_ids) && $mahasiswa_id > 0) {
        ...
        if ($verify_result['count'] != count($mata_kuliah_ids)) {
            ...
            // Hapus mata kuliah yang tidak dipilih
            $placeholders = implode(',', array_fill(0, count($mata_kuliah_ids), '?'));
            $stmt_delete = $conn->prepare("DELETE FROM krs WHERE id_mahasiswa = ? AND id_mata_kuliah NOT IN ($placeholders)");

            // Bind the parameters
            $params = array_merge([$mahasiswa_id], $mata_kuliah_ids);
            $stmt_delete->bind_param(str_repeat('i', count($params)), ...$params);

            if (!$stmt_delete->execute()) {
                $success = false;
                $message = "Gagal menghapus mata kuliah yang tidak dipilih: " . $stmt_delete->error;
                $message_type = 'danger';
            }
        }

        // Tambahkan mata kuliah yang baru dipilih
        if ($success) {
            foreach ($mata_kuliah_ids as $mata_kuliah_id) {
                // Jika mata kuliah belum ada di KRS, tambahkan
                if (!in_array($mata_kuliah_id, $mata_kuliah_terambil)) {
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="" method="POST" id="krsForm">
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead class="table-light">
                                    <tr>
                                        <th width="5%"><div class="form-check"><input class="form-check-input" type="checkbox" id="checkAll"></div></th>
                                        <th>Kode MK</th>
                                        <th>Mata Kuliah</th>
                                        <th>SKS</th>
                                        <th>Semester</th>
                                        ...
                                        <tr>
                                            <td>
                                                <div class="form-check">
                                                    <input class="form-check-input mk-checkbox" type="checkbox" name="mata_kuliah[]" value="<?= $row['id'] ?>"
                                                        <?= in_array($row['id'], $mata_kuliah_terambil) ? 'checked' : '' ?>>
                                                </div>
                                            </td>
                                            <td><?= $row['kode_mk'] ?></td>
                                            <td><?= $row['nama_mk'] ?></td>
                        ...
                        <button type="submit" class="btn btn-success mt-4">Simpan KRS</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateTotalSks() {
                ...
                checkboxes.forEach(checkbox => {
                    if (checkbox.checked) {
                        total += parseInt(checkbox.closest('tr').querySelector('td:nth-child(4)').textContent);
                    }
                });
                totalSks.textContent = `Total SKS: ${total}`;
            }
